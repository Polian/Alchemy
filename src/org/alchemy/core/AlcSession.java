...
class AlcSession implements ActionListener, AlcConstants {
    ...
    boolean saveSVG(File file) {
        ...
        int x = 0;
        int y = 0;
        if (Alchemy.canvas.getBufferedImage() != null){
            imageWidth = Alchemy.canvas.getBufferedImage().getWidth();
            imageHeight = Alchemy.canvas.getBufferedImage().getHeight();
            x = Alchemy.canvas.getImageLocation().x;
            y = Alchemy.canvas.getImageLocation().y;
        }
        svgGenerator.setSVGCanvasSize(new Dimension(imageWidth, imageHeight));
        ...
        // Set the view box to the size of the background image (if one exists)
        org.w3c.dom.Element root = svgGenerator.getRoot();
        String xPos = Integer.toString(x);
        String yPos = Integer.toString(y);
        root.setAttributeNS(null, "viewBox", xPos+" "+yPos+" "+imageWidth+" "+imageHeight);
        root.setAttributeNS(null, "shape-rendering", "crispEdges");
        
        // Remove the background image (if it exists)
        org.w3c.dom.NodeList images = root.getElementsByTagName("image");
        for(int i = 0; i < images.getLength(); i++){
            org.w3c.dom.Node cur = images.item(i);
            cur.getParentNode().removeChild(cur);
        }
        
        // Remove the background rectangle
        org.w3c.dom.NodeList rectList = root.getElementsByTagName("rect");
        org.w3c.dom.Node rect = rectList.item(0);
